...
@Component({
  ...
  imports: [SharedModule, SpkListCard, CommonModule, SpkDropdowns, SpkReusableTables, FormsModule,
    ReactiveFormsModule, NgbModule, SpkFlatpickr, SpkNgSelect],
...
open1(content1: any) {
  ...
  handleToggleSelectAll(checked: any) {
    this.Taskstable.forEach(task => task.checked = checked);
...
remove(id:string){
	const data = this.Taskstable.filter((x: { id: string }) => x.id !== id);
	this.Taskstable = data;
  }
