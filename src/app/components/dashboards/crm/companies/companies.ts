...
@Component({
  ...
  imports: [SharedModule, SpkDropdowns, SpkReusableTables, SpkNgSelect],
...
export class Companies {
    ...
    remove(id:number){
      const data = this.CompaniesTable.filter((x: { id: number }) => x.id !== id);
      this.CompaniesTable = data;
      }
      open(content: any) {
