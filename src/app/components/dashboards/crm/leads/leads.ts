...
@Component({
  selector: 'app-leads',
  imports: [SharedModule, SpkReusableTables, SpkDropdowns, SpkNgSelect],
...
export class Leads {
  ...
  remove(id:string){
    const data = this.Leadsdata.filter((x: { id: string }) => x.id !== id);
    this.Leadsdata = data;
    }
    open(content: any) {
