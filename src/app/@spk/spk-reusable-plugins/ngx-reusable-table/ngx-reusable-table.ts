import { Input, ChangeDetectorRef, Component, ViewChild, input } from '@angular/core';
...
@Component({
  selector: 'ngx-reusable-table',
  standalone: true,
  imports: [TableModule],

  templateUrl: './ngx-reusable-table.html',
  styleUrl: './ngx-reusable-table.scss'
})
export class NgxReusableTable {
   tableId = input<string>('');         // Dynamic table ID
   tableClass = input<string>('');      // Dynamic class for the table
  ...
  @Input() columns: Columns[] = [];          // Column definitions array
   ...
   paginationEnabled = input<boolean>(false);
   basicTable = input<boolean>(false);
